...
const cadastrarDisciplina = async (req, res) => {
  ...
  try {
    ...
    const result = await db.query(insertQuery, [professor_id, nome, carga_horaria, curso, semestre]);
    ...
    res.status(500).json({ error: 'Erro interno no servidor ao cadastrar disciplina.' });
  }
};
